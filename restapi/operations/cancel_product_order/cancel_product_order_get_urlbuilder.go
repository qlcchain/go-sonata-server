...
func (o *CancelProductOrderGetURL) Build() (*url.URL, error) {
	...
	var _path = "/productOrderManagement/v3/cancelProductOrder/{CancelProductOrderId}"

	cancelProductOrderID := o.CancelProductOrderID
	if cancelProductOrderID != "" {
		_path = strings.Replace(_path, "{CancelProductOrderId}", cancelProductOrderID, -1)
	} else {
		return nil, errors.New("cancelProductOrderId is required on CancelProductOrderGetURL")
	}
...
func (o *CancelProductOrderGetURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on CancelProductOrderGetURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on CancelProductOrderGetURL")
	}
