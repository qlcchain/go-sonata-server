...
func (o *QuoteManagementHubDeleteURL) Build() (*url.URL, error) {
	...
	var _path = "/quoteManagement/v2/hub/{HubId}"

	hubID := o.HubID
	if hubID != "" {
		_path = strings.Replace(_path, "{HubId}", hubID, -1)
	} else {
		return nil, errors.New("hubId is required on QuoteManagementHubDeleteURL")
	}
...
func (o *QuoteManagementHubDeleteURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on QuoteManagementHubDeleteURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on QuoteManagementHubDeleteURL")
	}
