...
func (o *ProductOrderGetURL) Build() (*url.URL, error) {
	...
	var _path = "/productOrderManagement/v3/productOrder/{ProductOrderId}"

	productOrderID := o.ProductOrderID
	if productOrderID != "" {
		_path = strings.Replace(_path, "{ProductOrderId}", productOrderID, -1)
	} else {
		return nil, errors.New("productOrderId is required on ProductOrderGetURL")
	}
...
func (o *ProductOrderGetURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on ProductOrderGetURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on ProductOrderGetURL")
	}
